...
def run_batch(args):
  ...
  if not osp.isfile(config_path):
    ...
    return
  log = osp.join(
    args.log_dir, "{0}_{1}_{2}_{3}_{4}_{5}{6}".format(args.match, args.limit, args.batch, args.filter, args.strategy, args.config,
                                                      "" if args.profile is None else "_{0}".format(args.profile)))
  log_dir = None
  ...
  common_flags = "-match_limit {0} -batch_size {1} -filter {2} -vertex_cover {3} -output_file {4}".format(
    args.limit, args.batch, args.filter, args.strategy, log)
  with open(config_path, 'r') as config_f:
    for line in config_f:
      if line.startswith("dataset"):
        continue
      run_query(line, args, log_dir, common_flags)
...
def run_query(config, args, log_dir, common_flags):
  ...
  cmd = "timeout {0} {1} -dataset {2} -query_size {3} -query_mode {4} -query_index {5} -match_order '{6}' {7} {8}".format(
    args.time_out, osp.join(args.bin_dir, args.match), flags[0], flags[1], flags[2], flags[3], flags[4].strip(), profile_flag, common_flags)
  os.system(cmd)
