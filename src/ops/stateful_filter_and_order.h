...
class StatefulFilterAndOrder {
 private:
  ...
  const LogicalNeighborhoodFilter* logical_filter_;
 ...
 public:
  ...
  static pair<LogicalNeighborhoodFilter*,std::vector<std::vector<VertexID>> > getCandidateSets(const Graph& g,const QueryGraph& q,std::string filter_string) { //only for test
    ...
    for (uint32_t v = 0; v < q.getNumVertices(); ++v) {
      ...
      if (filter_string.compare("ldf") && filter_string.compare("dpiso")) {
        scan->addFilter(std::make_unique<NLFFilter>(&q, v));
      }
      auto scan_ctx = scan->initScanContext(0);
      scan->scan(&g, &scan_ctx);
      candidates_sets[v] = std::move(scan_ctx.candidates);
      candidate_size[v] = candidates_sets[v].size();
    }

    if (filter_string.compare("ldf") && filter_string.compare("nlf")) {
      auto metadata = GraphMetadata(g);
      if (filter_string.compare("cfl") == 0) {
        logical_filter = new LogicalCFLFilter(metadata, &q, candidate_size);
      } else if (filter_string.compare("dpiso") == 0) {
        logical_filter = new LogicalDPISOFilter(metadata, &q, candidate_size);
      } else if (filter_string.compare("tso") == 0) {
        logical_filter = new LogicalTSOFilter(metadata, &q, candidate_size);
      } else if (filter_string.compare("gql") == 0) {
        logical_filter = new LogicalGQLFilter(&q);
      }
      ...
      for (auto& filter : physical_filters) {
        ...
        filter->filter(&g, &candidates_sets, &filter_ctx);
        candidates_sets[query_vertex].erase(
            std::remove_if(candidates_sets[query_vertex].begin(), candidates_sets[query_vertex].end(),
                           [invalid_vertex_id = INVALID_VERTEX_ID](VertexID & candidate) {
                             return candidate == invalid_vertex_id;
                           }),
            candidates_sets[query_vertex].end());
