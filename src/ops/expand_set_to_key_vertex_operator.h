...
class ExpandSetToKeyVertexOperator : public ExpandVertexOperator {
 ...
 protected:
  uint32_t profile(TraverseContext* ctx) {
    auto& input = (*(ctx->current_inputs_))[ctx->input_index_];
    uint32_t idx = 0;
    for (auto par : parents_) {
      uint32_t loop_num = 0;
      const auto& parent_match = input.getSet(query_vertex_indices_[par]);
      for (VertexID vid : *parent_match) {
        const auto& out_neighbors = ((G*)(ctx->current_data_graph_))->getOutNeighbors(vid, 0, idx);
  ...
  std::tuple<uint32_t, uint32_t, uint32_t> getMinimumParent(TraverseContext* ctx) {
    ...
    for (auto par : parents_) {
      ...
      for (auto set_vertex_id : *current_set) {
        current_size += ((G*)(ctx->current_data_graph_))->getVertexOutDegree(set_vertex_id, 0, idx);
      }
  ...
  inline uint32_t expandInner(std::vector<CompressedSubgraphs>* outputs, uint32_t batch_size, TraverseContext* ctx) {
    ...
    while (ctx->input_index_ < ctx->current_inputs_->size()) {
      ...
      if (min_parent_set_size < candidates_->size()) {
        ...
        output_num += fromSetNeighborStrategy<profile>(outputs, min_parent_vertex, min_parent_idx, ctx);
      } else {
        DLOG(INFO) << "fromCandidateStrategy";
        output_num += fromCandidateStrategy<profile>(outputs, ctx);
      }
      if
  ...
  template <QueryType profile>
  uint32_t fromCandidateStrategy(std::vector<CompressedSubgraphs>* outputs, TraverseContext* ctx) {
    auto& input = (*(ctx->current_inputs_))[ctx->input_index_];
    uint32_t output_num = 0;
    ...
    for (VertexID key_vertex_id : *candidates_) {
      ...
      auto key_out_neighbors = ((G*)(ctx->current_data_graph_))->getOutNeighbors(key_vertex_id, 0, 0);
      ...
      for (uint32_t set_vid : parents_) {
        ...
        if
          constexpr(
            ...
            key_out_neighbors = ((G*)(ctx->current_data_graph_))->getOutNeighbors(key_vertex_id, 0, parent_idx);
          }
  ...
  uint32_t fromSetNeighborStrategy(std::vector<CompressedSubgraphs>* outputs, QueryVertexID min_parent,
                                   uint32_t min_parent_idx, TraverseContext* ctx) {
    ...
    for (VertexID vid : *parent_match) {
      const auto& out_neighbors = ((G*)(ctx->current_data_graph_))->getOutNeighbors(vid, 0, min_parent_idx);
      for (uint32_t i = 0; i < out_neighbors.second; ++i) {
        ...
        if (visited.insert(key_vertex_id).second) {
          ...
          auto key_out_neighbors = ((G*)(ctx->current_data_graph_))->getOutNeighbors(key_vertex_id, 0, min_parent_idx);
          // TODO(by) hash key_out_neighbors
          CompressedSubgraphs new_output(input, key_vertex_id, same_label_set_indices_, set_pruning_threshold_);
          if (new_output.empty()) {
            continue;
...
// <<< for active pruning, should use same-label set indices
   ...
   * implementation. */
  void updateDistinctSICount(TraverseContext* ctx) {
    auto& input = (*(ctx->current_inputs_))[ctx->input_index_];
